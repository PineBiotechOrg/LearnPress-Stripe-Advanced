...
if ( ! class_exists( 'LP_Gateway_Stripe' ) ) {
	...
	class LP_Gateway_Stripe extends LP_Gateway_Abstract {
		...
		public function get_payment_form() {
			ob_start();
			$template = learn_press_locate_template( 'form.php', learn_press_template_path() . '/addons/stripe-payment/', LP_ADDON_STRIPE_ADVANCED_PAYMENT_TEMPLATE );
			include $template;
		...
		public function get_icon() {
			if ( empty( $this->icon ) ) {
				$this->icon = LP_ADDON_STRIPE_ADVANCED_PAYMENT_URL . 'assets/images/stripe.jpg';
			}
		...
		public function process_payment( $order ) {
			...
			if( is_array( $stripe ) && $stripe['free_coupon'] ) {
				...
				update_post_meta( $this->order->id, '_lp_transaction_id', $stripe['transaction_id'] );
		...
		public function send_to_stripe() {
			if ( $this->get_form_data() ) {
				...
				if( isset( LP()->session->coupon_code ) ) {

					$coupon_code = LP()->session->coupon_code;
					...
					if( array_key_exists( $coupon_code, $this->coupon_codes ) ) {
						...
						if( $amount == 0 ) {
							...
							return array( 'free_coupon' => true, 'transaction_id' => $transaction_id );
		...
		public function get_coupon_discount_percentage( $coupon_code ) {

			$discount = $this->coupon_codes[ $coupon_code ][ 'discount' ];

			if( ! ( absint( $discount ) >= 0 && absint( $discount ) <= 1 ) ) {
		...
		public function validate_fields() {
			...
			$coupon_code   = ! empty( $posted['coupon_code'] ) ? $posted['coupon_code'] : null;
			$error_message = array();
			if( ( ! empty( $coupon_code ) && $this->get_coupon_discount_percentage( $coupon_code ) < 1 ) || empty( $coupon_code ) ) {
